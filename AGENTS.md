...
- Must follow the `DRY` (do not repeat yourself) principle. No code repetition should exist for any reason.
...
- No `unsafe` code whatsoever, unless the user specifically requests so.
...
- Prefer **composition over inheritance**. Build behavior by combining small single-responsibility components rather than hierarchies.
...
- Only test application-specific business logic. Do not write tests for framework internals (i.e. serde serialization), third-party library behavior, or trivial glue code. Focus tests on: state transitions, input validation, error handling, and business rules.
</important_rules>
